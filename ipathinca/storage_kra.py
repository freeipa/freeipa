...
class KRAStorageBackend(LDAPStorageMixin):
    ...
    def init_schema(self):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.debug(
                    f"KRA base container already exists: {self.kra_base_dn}"
                )
            except errors.NotFound:
                logger.debug(
                    f"Creating KRA base container: {self.kra_base_dn}"
                )
    ...
    def _get_next_key_serial(self) -> int:
        ...
        with self._serial_lock, self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.debug(f"Allocated key serial number: {next_serial}")
                return next_serial

            except errors.NotFound:
                # Create config entry if it doesn't exist
                logger.debug(f"Creating KRA config entry: {config_dn}")
    ...
    def store_key(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(f"Storing key {key_id} for owner: {owner}")
            ...
            try:
                ...
                ldap.add_entry(entry)
                logger.info(f"Stored key {key_id} in KRA LDAP storage")
                return key_id

            except Exception as e:
                logger.error(
                    f"Failed to store key {key_id}: {e}", exc_info=True
                )
                raise
    ...
    def get_key(self, key_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(f"Retrieving key {key_id} from KRA LDAP storage")
            ...
            try:
                ...
                logger.warning(f"Key {key_id} not found in KRA LDAP storage")
                return None

            except Exception as e:
                logger.error(
                    f"Error retrieving key {key_id}: {e}", exc_info=True
                )
                raise
    ...
    def list_keys(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(f"Listing keys with filter: {ldap_filter}")
            ...
            try:
                ...
                logger.debug(f"Found {len(results)} keys")
                return results
    ...
    def update_key_status(self, key_id: str, status: str) -> bool:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(f"Updating key {key_id} status to: {status}")
            ...
            try:
                ...
                logger.info(f"Updated key {key_id} status to {status}")
                return True

            except errors.NotFound:
                logger.warning(f"Key {key_id} not found for status update")
                return False

            except Exception as e:
                logger.error(
                    f"Error updating key {key_id} status: {e}", exc_info=True
                )
                raise
    ...
    def delete_key(self, key_id: str) -> bool:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(f"Deleting key {key_id} from KRA LDAP storage")
            ...
            try:
                ...
                logger.error(
                    f"Error deleting key {key_id}: {e}", exc_info=True
                )
                raise
    ...
    def store_key_request(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(
                f"Storing key request {request_id}, type: {request_type}"
            )
            ...
            try:
                ...
                ldap.add_entry(entry)
                logger.info(f"Stored key request {request_id}")
                return request_id

            except Exception as e:
                logger.error(
                    f"Failed to store key request {request_id}: {e}",
                    exc_info=True,
                )
                raise
    ...
    def get_key_request(self, request_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.warning(f"Key request {request_id} not found")
                return None

            except Exception as e:
                logger.error(
                    f"Error retrieving key request {request_id}: {e}",
                    exc_info=True,
                )
                raise
