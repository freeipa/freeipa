...
class TransportKey:
    ...
    def generate_transport_key(self, ca_key, ca_cert, force: bool = False):
        ...
        logger.debug(
            f"Generating new KRA transport key ({self.key_size} bits) in NSSDB"
        )
        ...
        logger.debug(
            "Importing transport key and certificate to NSSDB: "
    ...
    def load_transport_key(self):
        ...
        if self.storage_backend and hasattr(
            ...
            try:
                ...
                logger.debug(
                    f"Could not retrieve HSM config for KRA {self.kra_id}: {e}"
                )
                hsm_config = None
        ...
        if hsm_config and hsm_config.get("enabled"):
            ...
            logger.debug(
                f"Loading KRA transport key from HSM for {self.kra_id}"
            )
            ...
            if is_internal_token(token_name):
                logger.error(
                    f"HSM enabled but token_name is internal: {token_name}"
                )
            ...
            logger.debug(
                ...
                f"private key (token={token_name}, label={key_label})"
            )
        else:
            # NSSDB path - extract from NSSDB (Dogtag-compatible)
            # Keys are stored in NSSDB, not as PEM files
            logger.debug(
                f"Extracting KRA transport key from NSSDB for {self.kra_id}"
            )
...
class StorageKey:
    ...
    def generate_storage_key(self, ca_key, ca_cert, force: bool = False):
        ...
        logger.debug(
            f"Generating new KRA storage key ({self.key_size} bits) in NSSDB"
        )
        ...
        # Import key and certificate to NSSDB
        logger.debug(
            "Importing storage key and certificate to NSSDB: "
            f"{storage_nickname}"
        )
        nssdb.import_key_and_cert(
            storage_nickname,
    ...
    def load_storage_key(self):
        ...
        if self.storage_backend:
            try:
                ...
                logger.debug(f"Could not check HSM config: {e}")

        if hsm_config and hsm_config.get("enabled"):
            # HSM path - use HSMPrivateKeyProxy for transparent PKCS#11 access
            logger.debug(f"Loading KRA storage key from HSM for {self.kra_id}")
            ...
            # Keys are stored in NSSDB, not as PEM files
            logger.debug(
                f"Extracting KRA storage key from NSSDB for {self.kra_id}"
            )
...
class KRA:
    ...
    def archive_secret(
        ...
        logger.debug(f"Archived secret for {owner}, key_id={key_id}")
        return key_id
    ...
    def retrieve_secret(self, key_id: str, requester: str) -> bytes:
        ...
        logger.debug(
            f"Retrieved secret {key_id} for {requester}, "
            f"owner={key_record.get('owner')}"
        )
