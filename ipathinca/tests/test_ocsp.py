...
class TestOCSPEndpoint:
    ...
    def test_ocsp_post(self, base_url, ca_cert_path):
        ...
        der, _ = _build_ocsp_request(ca_cert_path)
        if der is None:
    ...
    def test_ocsp_get(self, base_url, ca_cert_path):
        ...
        _, b64 = _build_ocsp_request(ca_cert_path)
        if b64 is None:
        ...
        url_b64 = b64.replace("+", "%2B").replace("/", "%2F").replace("=", "%3D")
        try:
            result = subprocess.run(
                [
                    "curl", "-sk", "--path-as-is", "--max-time", "15",
                    "-o", "/dev/null",
                    "-w", "%{http_code}",
