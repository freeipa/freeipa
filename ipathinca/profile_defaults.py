...
class SubjectNameDefault(Default):
    ...
    def apply(self, builder, csr, context: dict):
        ...
        from ipathinca.profile_parser import ProfileParser  # circular dep

        # Perform request variable extraction
        dn_str = ProfileParser.extract_request_variable(
            self.name_template, csr, context
        )
...
class SigningAlgDefault(Default):
    ...
    def _infer_from_key(self, public_key) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Using default signing algorithm from config: {default_alg}"
            )
            return default_alg
        except Exception as e:
            logger.warning(
                f"Could not read default_signing_algorithm from config: {e}"
            )
...
class AuthorityKeyIdentifierExtDefault(Default):
    ...
    def apply(self, builder, csr, context: dict):
        ...
        try:
            ...
            logger.warning(f"Failed to add AKI: {e}")
...
class ExtendedKeyUsageExtDefault(Default):
    ...
    def _parse_oid(self, oid_str: str):
        ...
        try:
            ...
            logger.warning(f"Invalid OID: {oid_str}")
            return None
    ...
    def apply(self, builder, csr, context: dict):
        ...
        for ad in self.access_descriptions:
            ...
            try:
                ...
                logger.warning(f"Invalid AIA method OID: {ad['method']}")
                continue
            ...
            if ad["location_type"] == "URIName":
                ...
                logger.warning(
                    f"Unsupported location type: {ad['location_type']}"
                )
                continue
    ...
    def apply(self, builder, csr, context: dict):
        ...
        try:
            ...
            if self.oid_str in context["extensions_added"]:
                logger.debug(
                    f"Extension {self.oid_str} already added, skipping"
                )
                return builder
            ...
            try:
                ...
                logger.debug(f"Added extension {self.oid_str} from CSR")
            ...
            logger.warning(
                f"Failed to copy user extension {self.oid_str}: {e}"
            )
...
class CommonNameToSANDefault(Default):
    ...
    def apply(self, builder, csr, context: dict):
        ...
        try:
            ...
            logger.debug(f"Added SAN extension with CN={cn}")

        except Exception as e:
            logger.warning(f"Failed to copy CN to SAN: {e}")
...
class SANToCNDefault(Default):
    ...
    def apply(self, builder, csr, context: dict):
        ...
        try:
            ...
            logger.warning(f"Failed to copy SAN to CN: {e}")
...
class OCSPNoCheckExtDefault(Default):
    ...
    def apply(self, builder, csr, context: dict):
        ...
        try:
            ...
            logger.warning(f"Failed to add OCSP No Check extension: {e}")
...
def create_default(class_id: str, params: Dict[str, Any]) -> Default:
    ...
    if not default_class:
        logger.warning(
            f"Unknown default class '{class_id}', using UserKeyDefault"
        )
    ...
    try:
        ...
        logger.error(f"Failed to create default {class_id}: {e}")
        raise
