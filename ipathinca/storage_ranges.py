...
class RangeStorage(BaseStorageBackend):
    ...
    def allocate_serial_range(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.info(
                f"Allocated serial range {begin_range}-{end_range} to replica "
                f"{replica_id}"
            )
    ...
    def update_range(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.info(
                f"Updated range {replica_id}-{old_begin_range}:"
                f" {current_begin}-{current_end} →"
                f" {current_begin}-{new_end_range}"
            )
    ...
    def get_range_info(self, replica_id: str, begin_range: int) -> Any:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.debug(f"Range {replica_id}-{begin_range} not found")
                return None
    ...
    def delete_range(self, replica_id: str, begin_range: int):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.info(f"Deleted serial range {replica_id}-{begin_range}")
            except errors.NotFound:
                logger.warning(f"Range {replica_id}-{begin_range} not found")
    ...
    def delete_replica_ranges(self, replica_id: str):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.info(
                    f"Deleted {deleted_count} serial ranges for replica "
                    f"{replica_id}"
                )

            except errors.NotFound:
                logger.debug(f"No ranges found for replica {replica_id}")
