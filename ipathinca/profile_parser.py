...
class ProfileParser:
    ...
    def _parse_policy(
        ...
        if not constraint_class or not default_class:
            logger.warning(
                f"Policy {policy_num} missing constraint or default class"
            )
            return None
        ...
        try:
            ...
            logger.error(f"Failed to create policy {policy_num} plugins: {e}")
            return None
    ...
    def substitute_variables(self, text: str, context: dict) -> str:
        ...
        logger.debug(f"substitute_variables input: {text}")
        logger.debug(f"substitute_variables context: {context}")
        ...
        logger.debug(f"substitute_variables output: {text}")
        return text
    ...
    def extract_request_variable(text: str, csr, context: dict) -> str:
        ...
        # Pattern: $$request.req_subject_name.cn$$
        ...
        def replacer(match):
            ...
            if not oid:
                logger.warning(f"Unknown subject attribute: {attr}")
            ...
            try:
                ...
                logger.warning(f"Failed to extract {attr}: {e}")
