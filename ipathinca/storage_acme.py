...
class ACMEStorageBackend(LDAPStorageMixin):
    ...
    def create_nonce(self, nonce_id: str, expires: datetime):
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                logger.debug(f"Created nonce: {nonce_id}")
        except Exception as e:
            logger.error(f"Failed to create nonce {nonce_id}: {e}")
            raise
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                try:
                    ...
                    logger.debug(f"Nonce not found: {nonce_id}")
                    return False
                ...
                if expires_str:
                    ...
                    if datetime.now(timezone.utc) > expires:
                        logger.debug(f"Nonce expired: {nonce_id}")
                ...
                try:
                    ...
                    logger.debug(
                        f"Nonce already consumed by concurrent request: "
                        f"{nonce_id}"
                    )
                    return False
                logger.debug(f"Validated and consumed nonce: {nonce_id}")
                return True

        except Exception as e:
            logger.error(f"Failed to validate nonce {nonce_id}: {e}")
            return False
    ...
    def create_account(self, account_id: str, jwk: Dict, contacts: List[str]):
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.add_entry(entry)
                logger.info(f"Created ACME account: {account_id}")
        except Exception as e:
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                if (
                    ...
                    logger.warning(f"Incomplete ACME account entry: {dn}")
                    return None
                try:
                    account_key = json.loads(account_key_val)
                except json.JSONDecodeError:
                    logger.warning(
                        f"Corrupt account key JSON for {account_id_val}"
                    )
                    return None
                return {
            ...
            logger.debug(f"Account not found: {account_id}: {e}")
            return None
    ...
    def get_account_by_jwk(self, jwk: Dict) -> Optional[Dict]:
        ...
        """
        try:
            # Import JWK class for thumbprint calculation
            from ipathinca.acme import JWK as ACMEJWKClass
            ...
            logger.error(f"Failed to search account by JWK: {e}")
            return None
    ...
    def create_order(
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.add_entry(entry)
                logger.info(
                    f"Created order: {order_id} for account {account_id}"
                )
        except Exception as e:
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                try:
                    ...
                    logger.warning(
                        f"Corrupt identifier JSON in order {order_id}"
                    )
                    return None
                return {
    ...
    def create_authorization(
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.add_entry(entry)
                logger.debug(f"Created authorization: {authz_id}")
        except Exception as e:
            logger.error(f"Failed to create authorization {authz_id}: {e}")
            raise
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                try:
                    ...
                    logger.warning(
                        f"Corrupt identifier JSON in authz {authz_id}"
                    )
                    return None
                return {
            ...
            logger.debug(f"Authorization not found: {authz_id}: {e}")
            return None
    ...
    def update_authorization_status(self, authz_id: str, status: str):
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.update_entry(entry)
                logger.info(
                    f"Updated authorization {authz_id} status to {status}"
                )
        except Exception as e:
            logger.error(f"Failed to update authorization {authz_id}: {e}")
            raise
    ...
    def create_challenge(
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.add_entry(entry)
                logger.debug(
                    f"Created {challenge_type} challenge: {challenge_id}"
                )
        except Exception as e:
            logger.error(f"Failed to create challenge {challenge_id}: {e}")
            raise
    ...
    def get_challenge(self, challenge_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"Challenge not found: {challenge_id}: {e}")
            return None
    ...
    def update_challenge_status(
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.update_entry(entry)
                logger.info(
                    f"Updated challenge {challenge_id} status to {status}"
                )
        except Exception as e:
    ...
    def store_certificate(self, cert_id: str, certificate_pem: str):
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                conn.add_entry(entry)
                logger.info(f"Stored certificate: {cert_id}")
        except Exception as e:
    ...
    def get_certificate(self, cert_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Certificate not found: {cert_id}: {e}")
            return None
