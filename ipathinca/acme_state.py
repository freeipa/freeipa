...
class ACMEStateManager:
    ...
    def _ensure_acme_structure(self, conn):
        ...
        try:
            ...
            logger.debug(f"ACME base container exists: {self.acme_base_dn}")
        except errors.NotFound:
            logger.info(f"Creating ACME base container: {self.acme_base_dn}")
        ...
        try:
            ...
            logger.debug(
                f"ACME config container exists: {self.acme_config_dn}"
            )
        except errors.NotFound:
            logger.info(
                f"Creating ACME config container: {self.acme_config_dn}"
            )
            entry = conn.make_entry(
                self.acme_config_dn,
    ...
    def is_enabled(self):
        ...
        try:
            ...
            logger.error(f"Failed to check ACME status: {e}")
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                if current_value_str.upper() != desired_value.upper():
                    ...
                    logger.info(f"ACME {'enabled' if enabled else 'disabled'}")
                else:
                    logger.info(
                        "ACME already "
            ...
            logger.error(
                f"Failed to {'enable' if enabled else 'disable'} ACME: {e}"
            )
            raise
