...
class KeyEncryption:
    ...
    def _get_master_key(self) -> bytes:
        ...
        try:
            ...
            if self.master_key_path.exists():
                ...
                logger.debug(
                    f"Loaded master encryption key from {self.master_key_path}"
                )
                return self._master_key

            # Generate new master key
            logger.info(
                "Generating new master encryption key at"
                f" {self.master_key_path}"
            )
    ...
    def encrypt_key(self, private_key_pem: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(
                f"Encrypted private key: {len(private_key_pem)} bytes -> "
    ...
    def decrypt_key(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(
                f"Decrypted private key: {len(encrypted_data)} bytes -> "
                f"{len(private_key_pem)} bytes"
            )
