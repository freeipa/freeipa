...
class JWK:
    ...
    def thumbprint(jwk_dict: Dict[str, Any]) -> str:
        ...
        if kty == "RSA":
            ...
            # Only include required fields in canonical order
            canonical = {
                "e": jwk_dict["e"],
                "kty": kty,
                "n": jwk_dict["n"],
        ...
        # Create JSON without spaces and hash it
...
class ACMEServer:
    ...
    def _generate_nonce(self) -> str:
        ...
        logger.debug(f"Generated nonce: {nonce[:20]}...")
        return nonce
    ...
    def _verify_nonce(self, nonce: str) -> bool:
        ...
        logger.debug(f"Verifying nonce: {nonce[:20]}...")
    ...
    def create_order(
        ...
        logger.info(
            f"Created ACME order {order.order_id} for account {account_id} "
            "in LDAP"
        )
        return order
    ...
    def respond_to_challenge(
        ...
        if challenge.status == "valid":
            logger.debug(f"Challenge {token} already valid, skipping")
            return challenge
    ...
    def finalize_order(
        ...
        logger.info(
            f"Finalized ACME order {order_id}, issued certificate "
            f"{serial_number}"
        )
        return order
    ...
    def _verify_http01_challenge(
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"HTTP-01 challenge returned status "
                    f"{response.status_code} for {identifier['value']}"
                )
                return False
            ...
            if secrets.compare_digest(
                ...
                logger.info(
                    "HTTP-01 challenge verification successful for "
                    f"{identifier['value']}"
                )
                return True

            logger.warning(
                f"HTTP-01 challenge response mismatch for "
                f"{identifier['value']}"
            )
            return False
        except requests.ConnectionError as e:
            logger.warning(
                f"HTTP-01 challenge connection failed for "
                f"{identifier['value']}: {e}"
            )
        except requests.Timeout:
            logger.warning(
                f"HTTP-01 challenge timed out for {identifier['value']}"
            )
        except Exception as e:
            logger.error(f"HTTP-01 challenge unexpected error: {e}")
        ...
        try:
            ...
            for answer in answers:
                if (
                    ...
                    logger.info(
                        "DNS-01 challenge verification successful for "
                        f"{identifier['value']}"
                    )
                    return True
        except dns.resolver.NXDOMAIN:
            logger.warning(
                f"DNS-01 challenge: no TXT record for "
                f"_acme-challenge.{identifier['value']}"
            )
        except dns.resolver.NoAnswer:
            logger.warning(
                f"DNS-01 challenge: no TXT answer for "
                f"_acme-challenge.{identifier['value']}"
            )
        except dns.resolver.Timeout:
            logger.warning(
                f"DNS-01 challenge: DNS timeout for {identifier['value']}"
            )
        except Exception as e:
            logger.error(f"DNS-01 challenge unexpected error: {e}")
    ...
    def revoke_certificate(self, certificate_der: bytes, reason: int = 0):
        ...
        try:
            ...
            logger.info(f"Revoked certificate {serial_number} via ACME")
