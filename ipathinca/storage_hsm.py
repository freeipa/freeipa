    ...
    def store_hsm_config(self, ca_id: str, config: Dict[str, Any]):
        ...
        with get_ldap_connection() as conn:
            ...
            try:
                ...
                logger.debug(
                    f"Stored HSM config for CA {ca_id}: {hsm_config_str}"
                )

            except errors.NotFound:
                logger.warning(
                    f"CA entry not found for {ca_id}, cannot store HSM config"
                )
                raise
    ...
    def get_hsm_config(self, ca_id: str) -> Optional[Dict[str, Any]]:
        ...
        with get_ldap_connection() as conn:
            ...
            try:
                ...
                if len(parts) != 2:
                    logger.warning(
                        f"Invalid HSM config format for CA {ca_id}: "
                        f"{hsm_config_str}"
                    )
                    return None
                ...
                if "ipaCaHSMMetadata" in entry:
                    ...
                    if metadata_bytes:
                        try:
                            ...
                            logger.warning(
                                f"Could not parse HSM metadata for CA "
                                f"{ca_id}: {e}"
                            )

                logger.debug(f"Retrieved HSM config for CA {ca_id}")
                return config

            except errors.NotFound:
                logger.debug(f"CA entry not found for {ca_id}")
                return None
    ...
    def delete_hsm_config(self, ca_id: str):
        ...
        with get_ldap_connection() as conn:
            ...
            try:
                ...
                logger.debug(f"Deleted HSM config for CA {ca_id}")
