...
/* Special case for discovering range type immediately after establishing the
 * trust by IPA framework.  For all other cases, call ipadb_reinit_mspac()
 * with force_reinit_mspac set to 'false' to make sure the information about
 * trusted domains is updated on a regular basis for all worker processes. */
static inline bool should_force_reinit(krb5_context context,
                                       struct ipadb_context *ipactx,
                                       krb5_const_principal client_princ)
{
    ...
    if (krb5_princ_size(context, client_princ) != 2)
        return false;

    pc0 = krb5_princ_component(context, client_princ, 0);
    if (strncmp(pc0->data, "HTTP", pc0->length) != 0)
        return false;

    pc1 = krb5_princ_component(context, client_princ, 1);
    host = ipactx->kdc_hostname;
    return ulc_casecmp(pc1->data, pc1->length, host, strlen(host), NULL, NULL,
                       &result) == 0;
}

krb5_error_code update_new_pac(krb5_context context, unsigned int flags,
                               krb5_pac old_pac, krb5_pac new_pac)
{
    ...
    for (size_t i = 0; i < num_buffers; i++) {
        /* krb5 handles updating DELEGATION_INFO for us. */
...
krb5_error_code ipadb_issue_pac(krb5_context context, unsigned int flags,
                                krb5_const_principal client_princ,
                                krb5_db_entry *client,
                                ...
                                krb5_pac new_pac)
{
    krb5_error_code ret;
    bool force;
    krb5_data pac_blob = { 0 };
    struct ipadb_context *ipactx;

    if (old_pac != NULL && (flags & KRB5_KDB_FLAG_CROSS_REALM)) {
        /* Failures here are generally internal ones, not
         * verification-related. */
        ret = krb5_pac_get_buffer(context, old_pac, KRB5_PAC_LOGON_INFO,
                                  &pac_blob);
        if (ret)
            goto done;

        ret = ipadb_check_logon_info(context, client_princ->realm, &pac_blob);
        if (ret)
            goto done;

        ret = update_new_pac(context, flags, old_pac, new_pac);
        if (ret)
            goto done;
        ...
        /* Be aggressive here. */
        force = should_force_reinit(context, ipactx, client_princ);
        (void)ipadb_reinit_mspac(ipactx, force);

        ret = ipadb_get_pac(context, client, flags, authtime, &new_pac);
        if (ret == ENOENT)
            ret = KRB5_PLUGIN_NO_HANDLE;
        if (ret)
            goto done;
