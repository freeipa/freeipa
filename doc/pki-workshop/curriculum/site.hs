...
main = hakyll $ do
  ...
  create ["CNAME"] $ do  -- GitHub Pages custom domain
    route idRoute
    compile $ do
      substBody <- loadBody "subst.txt"
      let substMap = parseSubstitutions substBody
      case M.lookup "CNAME_DOMAIN" substMap of
        Just domain -> makeItem (T.unpack domain)
        Nothing -> error "CNAME_DOMAIN not found in subst.txt"
