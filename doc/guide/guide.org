...
names_list = names('John Doe, John Lee, Brad Moe')
# names_list is ('John Doe', 'John Lee', 'Brad Moe')
...
class tank(Object):
    takes_params = (
        StrEnum('species*', label='Species', doc='Fish species',
                 values=('Angelfish', 'Betta', 'Cichlid', 'Firemouth')),
        Decimal('height', label='Height', doc='height in mm', default='400.0'),
        Decimal('width', label='Width', doc='width in mm', default='400.0'),
        Decimal('depth', label='Depth', doc='Depth in mm', default='300.0')
    )
...
def override_default_shell_cb(self, ldap, dn. 
    ...
    if 'loginshell' in entry_attrs:
        ...
        if entry_attrs['loginshell'] == default_shell:
            entry_attrs['loginshell'] = ['/dev/null']
...
- ~textui~ :: a backend to nicely format output to stdout which handles conversion from
              ...
              being.
   #+BEGIN_EXAMPLE
>>> entry = {'name' : 'Test example', 'age' : '100'}
