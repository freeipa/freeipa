...
and GID assignments, one has to disable subordinate ID usage. This should be
...
command and a new local ID range can be added to cover required part of the
...
Subordinate ID support can be disabled completely. This is done with `ipa
config-mod --addattr ipaconfigstring=SubID:Disable` command. After it is
disabled, subordinate ID range can be removed manually and it will not be
recreated.
