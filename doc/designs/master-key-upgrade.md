...
The next step is to set the new master key as the active one. To do so, use the hereunder command with the KVNO of the new master key (in this example, the KVNO is 2):

```
# kdb5_util use_mkey 2
```

ACBoth master keys should still be there afterwards, but now the new key has the `*` character. It signifies it is the new active key:
