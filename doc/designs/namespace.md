...
There will be nothing to prevent cn=admins from manually creating related permissions that grant access to namespaces but should be avoided because it will introduce both confusion (what is this for?) and could introduce privilege escalation possibilities.
...
* the number of permissions could heavily increase. This could severely impact API performance.
...
Two new 389-ds plugins will be required for enforcement.
...
The contribution did not allow usercat or hostcat to be set. Not allowing hostcat makes sense but its unclear what they are restricting without usercat unless they intend to add users one at a time or via groups. I believe usercat=all should be allowed.
...
* As a delegated user can I modify entries such that I can inadvertently grant other users elevated privileges? In particular SUDO and HBAC.
