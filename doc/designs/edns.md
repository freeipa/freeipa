...
We are not considering a scenario where DoT is enforced in the infrastructure where FreeIPA is being deployed. For this initial version, we allow unencrypted communication from clients during enrollment as well as encrypted communication. A stricter version will be supported by extending the deployment options later on.
