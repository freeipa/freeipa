...
In this initial version, we are not enforcing DoT within the infrastructure where FreeIPA is deployed. During client/replica enrollment, both unencrypted and encrypted communications will be allowed as default. However, for more restrictive scenarios, we need to add an option  such as --strict-dot to restrict unencrypted DNS traffic on port 53  when DoT is enabled.

When --strict-dot is enabled, it will require that DoT be pre-configured on clients and replicas prior to enrollment, ensuring they can connect to the server using port 853. This stricter approach will be supported in future versions by extending the deployment options, offering a more controlled deployment environment for users who require enforced DoT.
