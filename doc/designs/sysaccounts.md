...
| sysaccount-manage-reset | name    | Update `passSyncManagersDNs` configuration     |
...
`sysaccount-add`, `sysaccount-mod`, and `sysaccount-manage-reset`. This option is used to allow a
...
all those servers using `ipa sysaccount-manage-reset` command.
...
`ipa_pwd_extop` configuration, if any. However, it will only be done on this master. Thus, it is
recommended to remove the entry from all affected IPA servers using `ipa sysaccount-manage-reset`
first.

An example of running `ipa sysaccount-add my-app --random --privileged=true`:

```bash
$ ipa sysaccount-add my-app --random --privileged=true
ipa: WARNING: Password reset permission is local to server server.ipa.test.
Restart the Directory Server services on it. Run the 'sysaccount-manage-reset'
