...
privilege, to the `Security Architect` role. Members of `admins` group are not members of the
`Security Architect` role by default. Instead, they are explicitly added to the `Replication
...
- `System: Check System Accounts passwords`: allows to check whether system accounts have passwords defined
- `System: Modify System Accounts`: allows to update system account information, including a password
...
cause a password reset needs explicit buy-in, as described in the previous session.
