...
- LDAP objects whose DNs are stored in the `passSyncManagersDNs` attribute of the
  `cn=ipa_pwd_extop,cn=plugins,cn=config` LDAP entry can change passwords without marking them for a change.

The latter exception is used internally by the FreeIPA replication system to synchronize data from
Windows domain controllers with the help of PassSync plugin. The system accounts can be added to the
`passSyncManagersDNs` attribute to allow them to change passwords without marking them for a change.
...
In order to provide the management of the `passSyncManagersDNs` attribute, FreeIPA defines two
permissions:

- `Modify PassSync Managers Configuration` permission allows adding and removing DNs to and from the
  `passSyncManagersDNs` attribute.

- `Read PassSync Managers Configuration` permission allows reading the `passSyncManagersDNs`
  attribute.

Both these permissions are granted to the `Replication Administrators` privilege and, through that
privilege, to the `Security Architect` role. Members of `admins` group are members of the `Security
Architect` role by default as well.

### Access controls

To simplify administration of the system accounts, a new privilege `System Account Administrators`
is added. This privilege is granted by default to the `Security Architect` role.

The system account still needs to be permitted to modify user passwords through a role that includes
a corresponding permission ('System: Change User password'), through the privilege system. The
following sequence will allow a system account `my-app` to change user passwords:
...
$ ipa sysaccount-allow-reset 'my-app' --update-without-reset=true
```
...
| sysaccount-allow-reset | name    | Update `passSyncManagersDNs` configuration     |
...
The following commands provide an additional `--update-without-reset=TRUE|FALSE` option:
`sysaccount-add`, `sysaccount-mod`, and `sysaccount-allow-reset`. This option is used to allow a
...
to manage password synchronization (`passSyncManagersDNs` LDAP attribute) of the `ipa_pwd_extop`
plugin.
...
System accounts often used by a third-party software to modify passwords. If third-party
applications require communication with more than one IPA server, an update must be executed against
all those servers using `ipa sysaccount-allow-reset` command.

Due to the fact that `passSyncManagersDNs` configuration setting is not replicated across IPA
...
`ipa_pwd_extop` configuration, if any.

An example of running `ipa sysaccount-add my-app --random --update-without-reset=true`:

```bash
$ ipa sysaccount-add my-app --random --update-without-reset=true
ipa: WARNING: Password reset permission is local to server server.ipa.test.
Restart the Directory Server services on it. Run the 'sysaccount-allow-reset'
...
As IPA command line tool, `ipa`, does not currently provide an option to choose which server to run
against, it is recommended to run `passSyncManagersDNs` configuration change directly on the
corresponding IPA server.
