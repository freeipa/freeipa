...
class ProfileChangeMonitor(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stopped:
            try:
                ...
                if not self.stopped:
                    logger.error(
                        "Profile change monitor: Caught exception: %s",
                        e,
                        exc_info=True,
                    )
                    time.sleep(self.reconnect_delay)
    ...
    def _run_persistent_search(self):
        ...
        while not self.stopped:
            try:
                ...
                logger.warning(
                    "Profile change monitor: Error checking for changes: %s", e
                )
                time.sleep(poll_interval)
            ...
            with self.storage_backend._get_ldap_connection() as ldap:
                ...
                for entry in entries:
                    ...
                    if profile_id and entry_usn:
                        ...
                        logger.debug(
                            "Profile change monitor: Loaded %s with "
                            "entryUSN=%s",
                            profile_id,
                            entry_usn,
                        )
                ...
                logger.info(
                    "Profile change monitor: Loaded %s profiles", len(entries)
                )

        except errors.NotFound:
            logger.debug(
                "Profile change monitor: Profile container %s not found "
                "(expected during initial installation)",
                profiles_base,
            )
        except Exception as e:
            logger.error(
                "Profile change monitor: Failed initial load: %s",
                e,
                exc_info=True,
            )
    ...
    def _check_for_changes(self, profiles_base: DN):
        ...
        try:
            with self.storage_backend._get_ldap_connection() as ldap:
                ...
                for profile_id, data in current_profiles.items():
                    ...
                    if unique_id in self.deleted_unique_ids:
                        logger.debug(
                            "Profile change monitor: Ignoring %s with "
                            "deleted nsUniqueId %s",
                            profile_id,
                            unique_id,
                        )
                        continue
                    ...
                    if profile_id not in self.entry_usns:
                        # New profile added
                        logger.info(
                            "Profile change monitor: ADD - %s", profile_id
                        )
                ...
                for profile_id in deleted_profiles:
                    logger.info(
                        "Profile change monitor: DELETE - %s", profile_id
                    )
            ...
            logger.debug(
                "Profile change monitor: LDAP connection unavailable: %s", e
            )
        except Exception as e:
            logger.warning(
                "Profile change monitor: Error checking for changes: %s", e
    ...
    def _handle_add(self, profile_id: str, entry_usn: int):
        ...
        logger.debug("Profile change monitor: Added %s to cache", profile_id)
    ...
    def _handle_modify(self, profile_id: str, entry_usn: int):
        ...
        logger.debug(
            "Profile change monitor: Invalidated cache for %s", profile_id
        )
    ...
    def _handle_delete(self, profile_id: str):
        ...
        logger.debug(
            "Profile change monitor: Removed %s from cache", profile_id
        )
