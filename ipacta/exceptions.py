...
"""
Custom exception hierarchy for ipacta
...
class IpactaError(Exception):
    """
    Base exception for all ipacta errors

    All ipacta exceptions inherit from this base class, providing
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Example:
            >>> try:
            ...     raise IpactaError("Something went wrong",
            ...                          context={'foo': 'bar'})
            ... except IpactaError as e:
            ...     error_dict = e.to_dict()
            ...     print(error_dict)
            {'error': 'Something went wrong', 'error_type': 'IpactaError',
             'context': {'foo': 'bar'}}
        """
        return {
            "error": self.message,
...
class InvalidStateTransition(IpactaError):
    """
...
class CertificateLifecycleError(IpactaError):
...
class StorageError(IpactaError):
...
class CertificateRequestError(IpactaError):
...
class ProfileError(IpactaError):
...
class CAConfigurationError(IpactaError):
...
def format_error_response(
    exception: IpactaError, status_code: int = 400
) -> Dict[str, Any]:
    """
    Format exception as API error response

    Args:
        exception: IpactaError to format
