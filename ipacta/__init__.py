...
def set_global_config(config):
    """
    Set the global ipacta configuration

    This should be called once during backend initialization.

    Args:
        config: IpactaConfig or RawConfigParser-compatible object
    """
    global _global_config
    with _global_config_lock:
        _global_config = config
    logger.debug("Global ipacta config initialized")


def get_global_config():
    """
    Get the global ipacta configuration

    Returns:
        IpactaConfig or RawConfigParser-compatible object
    ...
    if config is None:
        raise CANotInitialized(
            "ipacta global config not initialized. "
...
def load_config(config_file=None):
    ...
    Delegates to ``IpactaConfig.from_file()`` so there is a single
    config-reading code path.

    Args:
        config_file: Path to config file (default: paths.IPACTA_CONF)

    Returns:
        IpactaConfig instance (RawConfigParser-compatible)
    ...
    return IpactaConfig.from_file(config_file)
