...
@pytest.mark.skipif(
    not os.path.exists("/etc/ipa/ipacta.conf"),
    reason="ipacta not deployed (no /etc/ipa/ipacta.conf)",
)
class TestLDAPConnectionPool:
    """Test LDAP connection pooling (requires running LDAP)."""

    def test_pool_creation(self, ipacta_config):
        """Pool creates minimum connections on init."""
        from ipacta.ldap_utils import LDAPConnectionPool
    ...
    def test_get_connection(self, ipacta_config):
    ...
    def test_close_all(self, ipacta_config):
