...
"""
Certificate and Request Pruning for ipacta
...
class PruningManager:
    ...
    def _ensure_pruning_structure(self, conn):
        ...
        try:
            ...
            logger.debug(
                "Pruning base container exists: %s", self.pruning_base_dn
            )
        except Exception:
            logger.info(
                "Creating pruning base container: %s", self.pruning_base_dn
            )
        ...
        try:
            ...
            logger.debug(
                "Pruning config container exists: %s", self.pruning_config_dn
            )
        except Exception:
            logger.info(
                "Creating pruning config container: %s", self.pruning_config_dn
            )
    ...
    def get_config(self):
        ...
        try:
            ...
            logger.error("Failed to get pruning configuration: %s", e)
        ...
        try:
            with get_ldap_connection() as conn:
                ...
                if changes_made:
                    ...
                    logger.info("Updated pruning configuration: %s", updates)
                else:
                    logger.info(
            ...
            logger.error("Failed to update pruning configuration: %s", e)
            raise
    ...
    def _parse_retention_time(self, retention_time, retention_unit):
        ...
        if retention_unit == "minute":
            ...
            logger.warning(
                "Unknown retention unit: %s, defaulting to days",
                retention_unit,
            )
            return timedelta(days=value)
    ...
    def run_pruning(self):
        ...
        logger.info(
            "Running pruning job: cert_cutoff=%s, request_cutoff=%s",
            cert_cutoff,
            request_cutoff,
        )
        ...
        try:
            if hasattr(self.storage, "delete_old_certificates"):
                ...
                logger.info("Deleted %s old certificates", cert_count)
            else:
                logger.warning(
                    "Storage backend does not support certificate pruning"
                )
        except Exception as e:
            logger.error("Error pruning certificates: %s", e, exc_info=True)
        ...
        try:
            if hasattr(self.storage, "delete_old_requests"):
                ...
                logger.info("Deleted %s old requests", request_count)
            else:
                logger.warning(
                    "Storage backend does not support request pruning"
                )
        except Exception as e:
            logger.error("Error pruning requests: %s", e, exc_info=True)
