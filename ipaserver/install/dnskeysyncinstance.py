...
def write_kryoptic_config():
    """Generate and write the Kryoptic PKCS#11 TOML configuration file."""
    kryoptic_db = os.path.join(paths.DNSSEC_TOKENS_DIR, "kryoptic.sql")
    kryoptic_conf_txt = (
        "# Kryoptic PKCS#11 configuration\n"
        "\n"
        "[[slots]]\n"
        "slot = 1\n"
        'dbtype = "sqlite"\n'
    ...
    logger.debug("Creating Kryoptic config file %s", paths.DNSSEC_KRYOPTIC_CONF)
    with open(paths.DNSSEC_KRYOPTIC_CONF, 'w') as f:
        os.fchmod(f.fileno(), 0o644)
        f.write(kryoptic_conf_txt)
...
class DNSKeySyncInstance(service.Service):
    ...
    def create_instance(self, fqdn, realm_name):
        ...
        self.step("setting up PKCS#11 token", self.__setup_hsm)
    ...
    def setup_named_openssl_conf(self):
        ...
        conf_file_dict = {
            ...
            'PKCS11_MODULE': paths.LIBKRYOPTIC_SO,
            'PKCS11_PIN': paths.DNSSEC_HSM_PIN,
        }
        if constants.NAMED_OPENSSL_ENGINE is not None:
            ...
            # pkcs11-provider to load PKCS#11 token
            if paths.CRYPTO_POLICY_OPENSSLCNF_FILE is None:
    ...
    def setup_ipa_dnskeysyncd_sysconfig(self):
        ...
        directivesetter.set_directive(
            sysconfig,
            'DNSSEC_HSM_PIN', paths.DNSSEC_HSM_PIN,
            quotes=False, separator='=')
    ...
    def __setup_hsm(self):
        ...
        if (token_dir_exists
                and os.path.exists(paths.DNSSEC_HSM_PIN)
                and os.path.exists(paths.DNSSEC_HSM_PIN_SO)):
            return
        ...
        # generate PINs for PKCS#11 token
        ...
        logger.debug("Saving user PIN to %s", paths.DNSSEC_HSM_PIN)
        with open(paths.DNSSEC_HSM_PIN, 'w') as f:
            # chown to ods:named
            constants.ODS_USER.chown(f.fileno(), gid=constants.NAMED_GROUP.gid)
            os.fchmod(f.fileno(), 0o660)
            f.write(pin)

        logger.debug("Saving SO PIN to %s", paths.DNSSEC_HSM_PIN_SO)
        ...
        logger.debug("Initializing PKCS#11 token")
        os.environ["KRYOPTIC_CONF"] = paths.DNSSEC_KRYOPTIC_CONF
        _ipap11helper.init_token(
            paths.LIBKRYOPTIC_SO,
            label=DNSSEC_TOKEN_LABEL,
            so_pin=pin_so,
            user_pin=pin,
        )
