...
class IPAACMEManage(AdminTool):
    ...
    def _detect_backend(self, state):
        """
        Detect which CA backend is in use (Dogtag or ipacta)

        Returns:
            str: 'ipacta' or 'dogtag'
        """
        try:
            ...
            if 'backend' in response_json:
                ...
                logger.debug(f"Detected CA backend: %s", backend)
                return backend
            return 'dogtag'
        except Exception as e:
            logger.warning("Failed to detect backend, assuming Dogtag: %s", e)
            return 'dogtag'
    ...
    def pruning(self):
        ...
        if backend == 'ipacta':
            self._pruning_ipacta(state)
        else:
            self._pruning_dogtag()

    def _pruning_ipacta(self, state):
        """Manage pruning for ipacta using REST API"""

        def ca_config_show(directive):
            """Get configuration value from ipacta REST API"""
            try:
                response = state.pki_client.connection.get(
                    '/ca/rest/pruning/config'
                )
                config = response.json()
                # Map cron to cronSchedule for ipacta
                if directive == 'cron':
                    return config.get('cronSchedule', '')
                return config.get(directive, '')
            except Exception as e:
                logger.error(f"Failed to get config for %s: %s", directive, e)
                return ''

        def ca_config_set(directive, value,
                          prefix='jobsScheduler.job.pruning'):
            """Set configuration value via ipacta REST API"""
            try:
                # Map cron to cronSchedule for ipacta
                if directive == 'cron':
                ...
                # Verify the update
                newvalue = ca_config_show(
                    directive if directive != 'cronSchedule' else 'cron'
                )
        ...
        def run_pruning():
            """Run the pruning job via ipacta REST API"""
            try:
        ...
        # ipacta doesn't have version restrictions
        # Random serial numbers are always used in ipacta
        ...
        if self.options.certsearchtimelimit is not None:
            ca_config_set(
                'certSearchTimeLimit', self.options.certsearchtimelimit
            )
        if self.options.certsearchsizelimit is not None:
            ca_config_set(
                'certSearchSizeLimit', self.options.certsearchsizelimit
            )
        if self.options.requestretention is not None:
            ca_config_set(
                'requestRetentionTime', self.options.requestretention
            )
        if self.options.requestretentionunit:
            ca_config_set(
                'requestRetentionUnit', self.options.requestretentionunit
            )
        if self.options.requestsearchsizelimit is not None:
            ca_config_set(
                'requestSearchSizeLimit', self.options.requestsearchsizelimit
            )
        if self.options.requestsearchtimelimit is not None:
            ca_config_set(
                'requestSearchTimeLimit', self.options.requestsearchtimelimit
            )
