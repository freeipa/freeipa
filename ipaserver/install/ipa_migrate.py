...
class IPAMigrate():
    ...
    def validate_options(self):
        ...
        if '.' not in hostname_value:
            self.handle_error(
                ...
                "remote server")
        # Remove all the dots, if it's a number its an IP not a FQDN
        hostname_value = hostname_value.replace('.', '')
        if isinstance(hostname_value, int):
        ...
        for attr in entry_attrs:
            ...
            if remove_member and attr == "member":
                ...
                for val in entry_attrs[attr]:
                    if not val.startswith("fqdn=" + self.args.hostname):
                        new_vals.append(val)
                    else:
                        remove_member = False
    ...
    def run(self):
        ...
        # Setup the arguments
        desc = f'IPA to IPA Migration Tool'
