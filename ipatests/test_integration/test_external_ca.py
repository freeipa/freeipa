...
class TestExternalCAInstall(IntegrationTest):
    ...
    def test_renew_external_ca_with_organization_identifier(self):
        ...
        # Create a custom subject with organizationIdentifier (OID 2.5.4.97)
        subject_with_org_id = x509.Name([
        ...
        # Set the issuer to our custom subject
        external_ca.issuer = subject_with_org_id

        # Build the root CA certificate with the custom subject
        builder = x509.CertificateBuilder()
        builder = builder.subject_name(subject_with_org_id)
        builder = builder.issuer_name(subject_with_org_id)
        ...
        root_ca = root_ca_cert.public_bytes(serialization.Encoding.PEM)
