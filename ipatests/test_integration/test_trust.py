...
class BaseTestTrust(IntegrationTest):
    ...
    def _ad_run_powershell(self, command, raiseonerr=True):
        ...
        :param str command: PowerShell command to execute.
        :param bool raiseonerr: If ``True``, raise when the remote command
            fails; if ``False``, return the result regardless of exit status.
        :returns: The object returned by ``Host.run_command`` on ``self.ad``.
        """
    ...
    def _ad_user_mod(self, username, options, ad_host=None, domain=None):
        ...
        ad_host = ad_host or self.ad
        domain = domain or self.ad_domain
        ad_usercn = self._ad_user_dn(username, domain)
