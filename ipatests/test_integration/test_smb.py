...
class TestSMB(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        cls.home_share = {
            'name': 'homes',
            'server_path': cls.smbserver.hostname,
            'unc': '//{}/homes'.format(cls.smbserver.hostname)
        }
    ...
    def smb_sanity_check(self, user, client_mountpoint, share):
        ...
        res = run_smb_client(['smbclient', '-k', share['unc'], '-c', 'dir'])
    ...
    def test_smb_access_for_ipa_user_at_ipa_client(self):
        ...
        self.mount_smb_share(user, password, self.home_share, mount_point)
        try:
            tasks.run_command_as_user(self.smbclient, user, ['kdestroy', '-A'])
            tasks.run_command_as_user(self.smbclient, user, ['kinit', user],
                                      stdin_text=password + '\n')
            self.smb_sanity_check(user, mount_point, self.home_share)
        finally:
            self.cleanup_mount(mount_point)

    @pytest.mark.usefixtures('enable_smb_client_dns_lookup_kdc')
    ...
    def test_smb_mount_and_access_by_different_users(self, samba_share):
        ...
        try:
            self.mount_smb_share(user1, password1, samba_share, mount_point)
            ...
            self.smb_sanity_check(user2, mount_point, samba_share)
        finally:
            self.cleanup_mount(mount_point)
    ...
    def test_smb_mount_fails_without_kerberos_ticket(self, samba_share):
        ...
        try:
            ...
            res = self.smbclient.run_command([
                'mount', '-t', 'cifs', samba_share['unc'], mountpoint,
                '-o', 'sec=krb5i,multiuser'
            ], raiseonerr=False)
