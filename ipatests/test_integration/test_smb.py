...
class TestSMB(IntegrationTest):
    ...
    def enable_smb_client_dns_lookup_kdc(self):
        host = self.smbclient
        save_file = tasks.create_temp_file(host)
        host.run_command(['cp', paths.KRB5_CONF, save_file])
        krb5_conf = host.get_file_contents(paths.KRB5_CONF, encoding='utf-8')
        krb5_conf = krb5_conf.replace(
            'dns_lookup_kdc = false', 'dns_lookup_kdc = true')
        host.put_file_contents(paths.KRB5_CONF, krb5_conf)
        yield
        host.run_command(['mv', save_file, paths.KRB5_CONF])

    @pytest.yield_fixture
    def samba_share(self):
        """Setup share outside /home om samba server."""
        share_path = '/srv/samba_shared'
        self.smbserver.run_command(['mkdir', share_path])
        self.smbserver.run_command(['chmod', '777', share_path])
        self.smbserver.run_command(
            ['chcon', '-t', 'samba_share_t', share_path])
        smbconf_save_file = tasks.create_temp_file(self.smbserver)
        self.smbserver.run_command(['cp', paths.SMB_CONF, smbconf_save_file])
        smb_conf = self.smbserver.get_file_contents(paths.SMB_CONF,
                                                    encoding='utf-8')
        ...
        [shared]
            ...
            browsable=yes
        '''.format(share_path))
        self.smbserver.put_file_contents(paths.SMB_CONF, smb_conf)
        self.smbserver.run_command(['systemctl', 'restart', 'smb'])
        yield {
            'name': 'shared',
            'server_path': share_path
        }
        self.smbserver.run_command(['mv', smbconf_save_file, paths.SMB_CONF])
        self.smbserver.run_command(['systemctl', 'restart', 'smb'])
        self.smbserver.run_command(['rmdir', share_path])

    def mount_smb_share(self, user, password, share_name, mountpoint):
        share_unc = '//{}/{}'.format(self.smbserver.hostname, share_name)
        ...
        self.smbclient.run_command([
            'mount', '-t', 'cifs', share_unc, mountpoint,
            '-o', 'sec=krb5i,multiuser'
        ])
        tasks.kdestroy_all(self.smbclient)

    def smb_sanity_check(self, user, client_mountpoint, server_share_path,
                         share_name):
        ...
        run_smb_client('printf "{}" > {}'.format(test_string, test_file_path))
        ...
        file_contents_at_server = self.smbserver.get_file_contents(
            '{}/{}'.format(server_share_path, test_file_path),
            encoding='utf-8')
        assert file_contents_at_server == test_string

        # check access using smbclient utility
        share_unc = '//{}/{}'.format(self.smbserver.hostname, share_name)
        res = run_smb_client(['smbclient', '-k', share_unc, '-c', 'dir'])
    ...
    def test_samba_service_listed(self):
        ...
        assert ('krbprincipalname: cifs/client1.testrelm.test@TESTRELM.TEST\n'
                in res.stdout_text)
        res = self.master.run_command(
    ...
    def test_smb_access_for_ipa_user_at_ipa_client(self):
        ...
        mount_point = '/mnt/smb'
        share_path = '/home/{}'.format(user)

        self.mount_smb_share(user, password, 'homes', mount_point)
        try:
            tasks.run_command_as_user(self.smbclient, user, ['kdestroy', '-A'])
            tasks.run_command_as_user(self.smbclient, user, ['kinit', user],
                                      stdin_text=password + '\n')
            self.smb_sanity_check(user, mount_point, share_path, 'homes')
        finally:
    ...
    def test_smb_access_for_ad_user_at_ipa_client(
        ...
        mount_point = '/mnt/smb'
        share_path = '/home/{}/{}'.format(
            self.ad.domain.name, self.ad_user_login)
    ...
    def test_smb_mount_and_access_by_different_users(self, samba_share):
        ...
        self.mount_smb_share(user1, password1, 'shared', mount_point)
        try:
            ...
            self.smb_sanity_check(
                user2, mount_point, samba_share['server_path'],
                samba_share['name'])
        finally:
            self.cleanup_mount(mount_point)

    def test_smb_mount_fails_without_kerberos_ticket(self, samba_share):
        share_unc = '//{}/{}'.format(
            self.smbserver.hostname, samba_share['name'])
        mountpoint = '/mnt/smb'
        try:
            tasks.kdestroy_all(self.smbclient)
            self.smbclient.run_command(['mkdir', '-p', mountpoint])
