...
class TestOTPToken(IntegrationTest):
    ...
    def test_2fa_only_with_password(self):
        ...
        try:
            ...
            cond = (tasks.parse_version(
                "2.9.6") < sssd_version < tasks.parse_version(
                "2.10") and osinfo.id == 'fedora')
            with (xfail_context(sssd_version < tasks.parse_version(
                    "2.10") and osinfo.id == 'fedora', reason=github_ticket)):
                result = ssh_2fa_with_cmd(master,
                                          self.master.external_hostname,
                                          USER3, PASSWORD, otpvalue=otpvalue,
                                          command="klist")
                print(result.stdout_text)
                assert ('Password authentication '
