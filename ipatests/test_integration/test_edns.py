...
def verify_queries_encrypted(master, replicas, clients, forwarder="1.1.1.1#853", dns_hostname="freeipa.org"):
    """
    ...
    for server in [master] + replicas:
        server.put_file_contents(os.path.join(paths.UNBOUND_CONF_DIR, "log.conf"), unbound_log_cfg)
...
class TestDOT_Relaxed(IntegrationTest):
    ...
    def test_dot_relaxed_dns_policy_without_certs(self):
        """
        This tests installs IPA server with --no-dnssec-validation option
        and relaxed policy.
        """
        args = [
            "--dns-over-tls",
    ...
    def test_dot_relaxed_dns_policy_with_external_ca(self):
        """
        This tests installs IPA server with --no-dnssec-validation
        option and relaxed policy with external CA.
        """
