...
def apply_enforced_dns_preconfig(host, master_ip, master_host, ca_cert_path, dest_cert_name):
    """
    Apply pre-configurations required for enforced DNS policy on client and replica.
    ...
    if osinfo.id in ['rhel', 'centos']:
        ...
        raise ValueError(f"Unsupported OS for enforced DNS policy: {osinfo.id}")
...
class TestDNSOverTLS_EnforcedPolicy_External_CA(IntegrationTest):
    ...
    def test_dot_enforced_dns_policy_with_external_ca(self):
        ...
        # Apply pre-configuration on client before installation (includes CA cert setup)
        apply_enforced_dns_preconfig(
            self.clients[0], self.master.ip, self.master,
        ...
        # Apply pre-configuration on replica before installation (includes CA cert setup)
        apply_enforced_dns_preconfig(
            self.replicas[0], self.master.ip, self.master,
            cert_dest, "certificate.pem"
        )
