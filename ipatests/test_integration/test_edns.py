    ...
    def test_queries_encrypted(self):
        ...
        for server in [self.master, self.replicas[0]]:
            server.put_file_contents("/etc/unbound/conf.d/log.conf",
                                     unbound_log_cfg)
            Firewall(server).enable_service("dns-over-tls")
        ...
        # Sleep before running dig to prevent logs from previous queries
        time.sleep(60)
        self.master.run_command(["journalctl", "--flush", "--rotate",
                                "--vacuum-time=1s"])
        time.sleep(60)
