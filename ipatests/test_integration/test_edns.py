...
def setup_edns_env(host, setup_firewall=False, install_package=True):
    """
    Setup DNS over TLS environment for a host.

    Args:
        host: Host object to configure

        setup_firewall: Whether to enable dns-over-tls
        firewall service (default: False)

        install_package: Whether to install encrypted
        DNS package (default: True)
    """
...
def setup_dns_over_tls_environment(cls):
    ...
    """
    # Setup firewall and packages for master
    setup_edns_env(cls.master, setup_firewall=True, install_package=True)

    # Setup firewall and packages for replicas
    setup_edns_env(cls.replicas[0], setup_firewall=True, install_package=True)

    # Setup packages for clients (no firewall)
    setup_edns_env(cls.clients[0], setup_firewall=False, install_package=True)
