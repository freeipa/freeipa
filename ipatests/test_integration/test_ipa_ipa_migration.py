...
class TestIPAMigrateCLIOptions(MigrationTest):
    ...
    def test_ipa_sigden_plugin_fail_error(self, empty_log_file):
        ...
        not seen during migrate stage-mode
        """
    ...
    def test_automountlocation_is_migrated(self, add_automount):
        """
    ...
    def test_check_vault_is_migrated(self):
        """
        ...
        assert cmd.returncode != 0
        assert CMD_OUTPUT not in cmd.stdout_text
...
class TestIPAMigrationStageMode(MigrationTest):
    ...
    def test_ipa_migrate_stage_mode_new_user(self):
        ...
        tasks.user_add(self.master, username, password=original_passwd)
        install_msg = self.replicas[0].get_file_contents(
            paths.IPA_MIGRATE_LOG, encoding="utf-8"
        )
...
class TestIPAMigrationWithADtrust(IntegrationTest):
    ...
    def test_check_ad_attributes_migrate_prod_mode(self):
        ...
        CMD_LOG = "Trust direction: Trusting forest"
        run_migrate(
            self.replicas[0],
            "prod-mode",
            self.master.hostname,
    ...
    def test_check_domain_sid_is_migrated(self):
        ...
        mirgrated from a remote server having trust with AD
        ...
        assert "Domain: {}".format(base_dn) in cmd.stdout_text
        assert "Security Identifier: S-1-5-21-" in cmd.stdout_text
        assert "IPA AD trust agents: {}".format(self.master.hostname)
    ...
    def test_check_ad_idrange_is_migrated(self):
        ...
        to local IPA server
        """
        ad_domain = str(self.ad_domain)
        cmd = self.replicas[0].run_command(
            ["ipa", "idrange-find"]
        )
        assert "Range name: {}_id_range".format(ad_domain) in cmd.stdout_text
        assert "Domain SID of the trusted domain: S-1-5-21-" in cmd.stdout_text
