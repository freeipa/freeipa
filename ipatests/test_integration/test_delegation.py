...
class TestDelegationShow(IntegrationTest):
    ...
    def test_delegation_show_fail_nonexist_delegation_by_name(self):
    ...
    def test_delegation_show_fail_nonexist_delegation_with_all_option(self):
    ...
    def test_delegation_show_fail_nonexist_delegation_with_raw_option(self):
        """Test fail to show nonexistent delegation by name with option raw"""
        tasks.kinit_admin(self.master)
    ...
    def test_delegation_show_fail_nonexist_delegation_all_and_raw_options(
        self,
    ):
        """Test fail to show nonexistent delegation by name with options all and raw"""
        tasks.kinit_admin(self.master)
...
class TestDelegationUser(IntegrationTest):
    ...
    def test_delegation_user_employee_fail_change_address_attrs(self):
        """Test kinit as employee and fail to change address attrs for employee"""
        # Kinit as employee
        self.master.run_command(["kinit", "emp0001"], stdin_text="passw0rd1\n")
        ...
        for attr_option, attr_name in address_changes:
            ...
            assert (
                result.returncode == 1
            ), f"Employee should not be able to change other employee {attr_name}"
...
class TestDelegationBZ(IntegrationTest):
    ...
    def test_delegation_bz_784468(self):
        ...
        # --membergroup=managers --group=employees (managers can edit employees)
...
class TestDelegationClientSide(IntegrationTest):
    ...
    def test_delegation_error_messages(self):
        """Test that delegation error messages are user-friendly"""
        tasks.kinit_admin(self.master)
        ...
        self.master.run_command(
            ...
            stdin_text="errorpass123\nerrorpass123\n",
        )

        # Test as non-privileged user
        self.master.run_command(
            ["kinit", "erroruser"], stdin_text="errorpass123\n"
        )

        # Test various error scenarios and check for helpful messages
        test_scenarios = [
            # (command, expected_error_keywords)
            (
                ["user-mod", "admin", "--first=Hacker"],
        ...
        for command, error_keywords in test_scenarios:
            res = self.master.run_command(
                ["ipa"] + command, raiseonerr=False
            )
            assert res.returncode != 0

            # Check that error message contains helpful keywords
            error_text = res.stderr_text.lower()
            found_keyword = any(
                keyword in error_text for keyword in error_keywords
            )
            assert (
                found_keyword
            ),f"Error message should contain one of{error_keywords}, got: {res.stderr_text}"

        # Cleanup
        tasks.kinit_admin(self.master)
        self.master.run_command(
            ["ipa", "user-del", "erroruser"], raiseonerr=False
        )
    ...
    def test_delegation_permission_inheritance(self):
        ...
        # Test that child user can also modify target (through group inheritance)
        self.master.run_command(
