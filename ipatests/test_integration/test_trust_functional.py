...
class TestTrustFunctionalUser(BaseTestTrust):
    ...
    def test_kinit_disabled_account(self):
        ...
        Related: BZ1162486 (account locked), test 0006 (subdomain)
        """
        test_user = "disabledtestuser"
        test_password = "Secret123"
        for ad_host, ad_domain in [(self.ad, self.ad_domain),
                                   (self.child_ad, self.ad_subdomain)]:
            try:
                self._ad_user_add(test_user, test_password, ad_host)
                self._ad_user_disable(test_user, ad_host)

                tasks.kdestroy_all(self.clients[0])
                tasks.clear_sssd_cache(self.clients[0])

                disabled_user = f"{test_user}@{ad_domain}"
                result = self.clients[0].run_command(
                    ['kinit', disabled_user],
                ...
                output = f"{result.stdout_text}{result.stderr_text}"
                assert result.returncode != 0, f"kinit should fail for {ad_domain}"
                assert "credentials have been revoked" in output or "locked" in output.lower(), (
                    f"Expected 'revoked' or 'locked' in output for {ad_domain}"
                )
            finally:
    ...
    def test_kinit_expired_account(self):
        ...
        for ad_host, ad_domain in [(self.ad, self.ad_domain),
            ...
            try:
                self._ad_user_add(test_user, test_password, ad_host)
                self._ad_user_expire(test_user, ad_host)
                ...
                assert "credentials have been revoked" in output, (
                    f"Expected 'revoked' or 'expired' in output for {ad_domain}"
                )
            finally:
                self._ad_user_del(test_user, ad_host)
    ...
    def test_passwd_change_by_user(self):
        ...
        Related: BZ870238, test 0010 (subdomain)
        """
    ...
    def test_child_user_in_forest_group(self):
        ...
        for user in [self.subaduser, self.aduser]:
            self._ssh_with_password(
        ...
        time.sleep(5)
        # Check if subdomain user appears in top domain group
    ...
    def test_ad_user_in_posix_group_fully_qualified(self):
        ...
        for users, domain_desc in test_cases:
            ...
            try:
                ...
                for user in users:
                    self.master.run_command([
                        'ipa', 'group-add-member', ext_group,
                        '--users=', '--groups=', f'--external={user}'
                    ])
                time.sleep(60)
    ...
    def test_ad_user_in_multiple_groups(self):
        ...
        Tests that id and getent commands display secondary groups
        for trusted AD users from both top domain and subdomain.
        ...
        for user, domain in test_cases:
            # SSH with password to resolve user
            self._ssh_with_password(
                user, self.clients[0].hostname, 'Secret123'
            )
            time.sleep(10)
            # Check user appears in multiple AD groups
            for grp_name in ['testgroup', 'testgroup1']:
                ...
                if result.returncode == 0:
                    assert user in result.stdout_text, (
                        f"User {user} not found in group {ad_grp}: "
