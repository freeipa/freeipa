...
class TestTrustFunctionalSSH(BaseTestTrust):
    ...
    def test_ssh_gssapi_credential_forwarding(self):
        ...
        tasks.kinit_as_user(client, self.testuser1, self.ad_user_password)
        result = client.run_command([
            'ssh', '-o', 'StrictHostKeyChecking=no', '-K',
            '-l', self.testuser1, client.hostname,
            '--',
            ...
            '-l', self.testuser2, client.hostname, 'id',
        ])
        assert self.testuser2 in result.stdout_text
    ...
    def test_ssh_su_second_ad_user_in_session(self):
        ...
        assert self.testuser2 in output
            ...
            try:
                ...
                with spawn_ssh_interactive(
                    ...
                    expect_password_change_prompts(
                        test, self.ad_user_first_password, ad_user_new_password
                    )
                    test.expect(r'.*[$#] ')
    ...
    def test_ipa_trust_func_short_principal(self):
        ...
        if not exercised:
            pytest.skip(
                "IPA realm/DNS name not long enough vs trusted AD domain(s); "
                "BZ878262 short-principal scenario does not apply in this "
                "topology.")
