...
class TestMkeyUpgrade(IntegrationTest):
    ...
    def install(cls, mh):
        cls.master.put_file_contents(
            ...
            'export IPAPLATFORM_OVERRIDE=enctype_upgrade')
        tasks.install_master(cls.master, setup_dns=False)
        tasks.install_replica(cls.master, cls.replicas[0], setup_dns=False)

    def test_old_active_mkey(self):
        self.master.run_command(['env'])
        self.master.run_command(
            "kdb5_util list_mkeys | "
            "grep -Eq '^KVNO: 1, Enctype: aes256-cts-hmac-sha1-96, .* \\*$'")
    ...
    def test_new_inactive_mkey(self):
        self.master.run_command(
            "kdb5_util list_mkeys | "
            "grep -Eq '^KVNO: 2, Enctype: aes256-cts-hmac-sha384-192, '")
        self.replicas[0].run_command(
            "kdb5_util list_mkeys | "
            "grep -Eq '^KVNO: 2, Enctype: aes256-cts-hmac-sha384-192, '")
    ...
    def test_used_old_mkey(self):
        self.master.run_command(
            f"kadmin.local getprinc ldap/{self.master.hostname} | "
            "grep -Eq '^MKey: vno 1$'")
        self.replicas[0].run_command(
            f"kadmin.local getprinc ldap/{self.master.hostname} | "
            "grep -Eq '^MKey: vno 1$'")
    ...
    def test_only_new_mkey(self):
        self.master.run_command("kdb5_util list_mkeys | ! grep -Eq '^KVNO: 1,'")
        self.replicas[0].run_command(
            "kdb5_util list_mkeys | ! grep -Eq '^KVNO: 1,'")

    @classmethod
    def uninstall(cls, mh):
        cls.master.remove_file('/etc/profile.d/ipaplatform.sh')
        super().uninstall(mh)
