...
def set_kdcconf_supported_enctypes(host, enctypes):
    host.run_command([
        'sed', '-i',
        r's/^\( *supported_enctypes *=\).*$/\1 %s/' % (' '.join(enctypes)),
        KDCCONF_PATH
    ])
...
class KeyTypeTest(IntegrationTest):
    ...
    def test_reverse_keys_order(self, kvnos):
        ...
        set_kdcconf_supported_enctypes(self.master, reversed_configured)
        ...
        set_kdcconf_supported_enctypes(self.master, self.configured)
        tasks.restart_ipa_server(self.master)
    ...
    def test_different_permitted_enctypes(self, kvnos):
        ...
        for _, cls in inspect.getmembers(
            ...
            set_kdcconf_supported_enctypes(self.master, cls.configured)
            tasks.restart_ipa_server(self.master)
