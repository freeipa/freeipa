...
def configure_starttls(host):
    ...
    host.run_command(
        [
            "rm",
            "-f",
            os.path.join(host.paths.OPENSSL_PRIVATE_DIR, SMTPD_KEY),
            os.path.join(host.paths.OPENSSL_CERTS_DIR, SMTPD_CERT),
        ]
    )
...
def configure_ssl_client_cert(host):
    ...
    host.run_command(
        [
            "rm",
            "-f",
            os.path.join(host.paths.OPENSSL_PRIVATE_DIR, SMTP_CLIENT_KEY),
            os.path.join(host.paths.OPENSSL_CERTS_DIR, SMTP_CLIENT_CERT),
        ]
    )
    ...
    host.run_command(["ipa-getcert", "request",
                      "-f",
                      os.path.join(
                          host.paths.OPENSSL_CERTS_DIR, SMTP_CLIENT_CERT
                      ),
                      "-k",
                      os.path.join(
                          host.paths.OPENSSL_PRIVATE_DIR, SMTP_CLIENT_KEY
                      ),
    ...
    postconf(host, f"smtpd_tls_CAfile = {host.paths.IPA_CA_CRT}")
...
class TestEPN(IntegrationTest):
    ...
    def uninstall(cls, mh):
        ...
        for cert in [
            os.path.join(cls.master.paths.OPENSSL_CERTS_DIR, SMTPD_CERT),
            os.path.join(cls.master.paths.OPENSSL_CERTS_DIR, SMTP_CLIENT_CERT),
        ]:
        ...
        cls.master.run_command(
            [
                ...
                "-f",
                os.path.join(cls.master.paths.OPENSSL_CERTS_DIR, SMTPD_CERT),
                os.path.join(cls.master.paths.OPENSSL_PRIVATE_DIR, SMTPD_KEY),
                os.path.join(
                    cls.master.paths.OPENSSL_CERTS_DIR, SMTP_CLIENT_CERT
                ),
                os.path.join(
                    cls.master.paths.OPENSSL_PRIVATE_DIR, SMTP_CLIENT_KEY
                ),
            ]
        )
    ...
    def test_EPN_config_file(self):
        ...
        if self.master.osinfo.platform != "fedora":
    ...
    def test_EPN_ssl_client_cert(self, cleanupmail):
        ...
        epn_conf = (SSL_EPN_CONF + CLIENT_CERT_EPN_CONF).format(
            ...
            client_cert=os.path.join(
                self.master.paths.OPENSSL_CERTS_DIR, SMTP_CLIENT_CERT
            ),
            client_key=os.path.join(
                self.master.paths.OPENSSL_PRIVATE_DIR, SMTP_CLIENT_KEY
            ),
