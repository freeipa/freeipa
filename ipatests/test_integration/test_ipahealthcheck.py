    ...
    def test_ipa_trust_check_exists(self):
        ...
        result = self.master.run_command(
            ["ipa-healthcheck", "--source",
             "ipahealthcheck.ipa.trust", "--all"]
        )
        for check in ipatrust_checks:
            if check in not_a_trust_agent:
                assert "Skipped. Not a trust agent" in result.stdout_text
            elif check in not_a_trust_controller:
                assert "Skipped. Not a trust controller" in result.stdout_text
            else:
                assert check in result.stdout_text
