...
class TestKeyTypesAfterInstall(IntegrationTest):
    ...
    def test_ldap_enctypes_present(self):
        """LDAP kerberos container should list enc:salt types matching
        the system permitted enctypes from crypto-policies."""
        # Map canonical MIT enctype names to the shorter LDAP attribute
        # names used in krbSupportedEncSaltTypes/krbDefaultEncSaltTypes.
        # Derived dynamically from permitted_enctypes so the test works
        # under any crypto policy (DEFAULT, FIPS, FUTURE, etc.).
        enctype_to_ldap = {
        ...
        expected_enctypes = []
        for enc in self.permitted_enctypes:
            ldap_name = enctype_to_ldap.get(enc)
            if ldap_name:
                expected_enctypes.append(f'{ldap_name}:normal')
                expected_enctypes.append(f'{ldap_name}:special')
