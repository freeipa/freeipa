...
class TestHBACFunctional(IntegrationTest):
    ...
    def install(cls, mh):
        ...
        tasks.hbacrule_enable(cls.master, "allow_all", raiseonerr=False)
    ...
    def test_hbacsvc_master_001(self, request):
        ...
        tasks.kdestroy_all(self.clients[1])
    ...
    def test_hbacsvc_master_002(self):
        ...
        # Test both clients
        for client in [self.clients[0], self.clients[1]]:
            tasks.clear_sssd_cache(client)
            client.run_command(
                ["getent", "-s", "sss", "passwd", self.USER_1]
            )
    ...
    def test_hbacsvc_master_003(self, request):
        ...
        # Create rule: allow ftp access to user1 on master through a service
        # group
        tasks.hbacrule_add(self.master, self.HBAC_RULE)
    ...
    def test_hbacsvc_master_015_1(self, request):
        ...
        for client in [self.clients[0], self.clients[1]]:
            tasks.clear_sssd_cache(client)
            client.run_command(["id", self.USER_1], raiseonerr=False)
            client.run_command(
                ["getent", "-s", "sss", "passwd", self.USER_1]
            )
            tasks.kdestroy_all(client)
            assert self.ssh_auth_failure(
                self.USER_1, self.USER_PASSWORD, client
            )
