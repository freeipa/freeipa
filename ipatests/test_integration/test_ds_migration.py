...
def migrate_ds(master, extra_args, password, raiseonerr=True):
    """Run ``ipa migrate-ds`` with *extra_args*."""
    return master.run_command(
        ["ipa", "migrate-ds"] + extra_args,
        stdin_text=password,
...
def kerberos_keys_available(master, uid):
...
def migrate_bind(master, username, password):
    """LDAP simple bind as *username* to trigger key generation."""
    base_dn = master.domain.basedn
...
class TestDSMigrationConfig(IntegrationTest):
    ...
    def test_hashedpwd_migration(self):
        ...
        try:
            ...
            assert not kerberos_keys_available(self.master, user1)
            assert not kerberos_keys_available(self.master, user2)

            migrate_bind(self.master, user1, user1pwd)
            assert kerberos_keys_available(self.master, user1)
            tasks.kinit_as_user(self.master, user1, user1pwd)

            migrate_bind(self.master, user2, user2pwd)
            assert kerberos_keys_available(self.master, user2)
...
class TestDSMigrationOptions(IntegrationTest):
    """
    ...
    def install(cls, mh):
        ...
        # Set up 389-ds on the client with base migration test data
        _setup_389ds_on_client(
        ...
        # Enable migration mode and disable compat plugin
        tasks.kinit_admin(cls.master)
        cls.master.run_command(
            ["ipa", "config-mod", "--enable-migration", "TRUE"],
        )
    ...
    def test_invalid_directory_server_unreachable(self):
        ...
        result = migrate_ds(self.master, args, pwd, raiseonerr=False)
    ...
    def test_invalid_schema_option(self):
        ...
        out = result.stdout_text + result.stderr_text
        assert result.returncode == 1, out
        assert "invalid" in out.lower() and "schema" in out.lower(), out
    ...
    def test_invalid_bind_password(self):
        ...
        result = migrate_ds(self.master, args, "badpWd882", raiseonerr=False)
        out = result.stdout_text + result.stderr_text
        assert result.returncode == 1, out
        assert (
            "invalid credentials" in out.lower()
            or "insufficient access" in out.lower()
        ), out

    def test_bind_dn_non_directory_manager(self):
        """migrate-ds using a regular user bind DN
        instead of Directory Manager must fail because the
        remote 389-ds has no ACIs granting search access."""
        tasks.kinit_admin(self.master)
        ...
        result = migrate_ds(
            self.master, args, "fo0m4nchU", raiseonerr=False
        )
        out = result.stdout_text + result.stderr_text
        assert result.returncode == 2, out
        assert "user LDAP search did not return any result" in out, out
    ...
    def test_exclude_user(self):
        ...
        try:
            migrate_ds(self.master, args, pwd, raiseonerr=True)
