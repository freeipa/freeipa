...
def is_389_ds_version_enough(host):
    ds_version = ''
    cmd = host.run_command(['/usr/sbin/ns-slapd', '-v'], raiseonerr=False)
    for line in cmd.stdout_text.split('\n'):
        if '389-Directory' in line:
            ds_version = line.split()[0].split('/')[1]
    return bool(parse_version(ds_version) >= parse_version("1.3.9.1"))
...
class TestAutounmembership(IntegrationTest):
    ...
    def test_modify_user_entry_unmembership(self):
        ...
        """
        if is_389_ds_version_enough(self.master) != True:
            raise unittest.SkipTest('Auto-unmember feature is'
                                    ' not available on fedora 27')
        try:
            # testcase setup
