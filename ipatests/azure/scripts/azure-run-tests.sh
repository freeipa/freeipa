...
IPA_TESTS_TO_DESELECT_VARNAME="IPA_TESTS_TO_DESELECT_${PROJECT_ID}"
IPA_TESTS_TO_DESELECT="${!IPA_TESTS_TO_DESELECT_VARNAME:-}"
[ -n "$IPA_TESTS_TO_DESELECT" ] && \
IPA_TESTS_TO_DESELECT=$(eval "echo --deselect\ {$(echo $IPA_TESTS_TO_DESELECT | sed -e 's/[ \t]+*/,/g')}" | tr -d '{}')
...
{ podman exec -t \
    ...
    --env IPA_TESTS_TO_DESELECT="$IPA_TESTS_TO_DESELECT" \
