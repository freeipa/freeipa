...
host1 = 'ipatesthost.%s' % api.env.domain
host_dn1 = DN(('fqdn',host1),('cn','computers'),('cn','accounts'),
              api.env.basedn)

unknown_host = 'unknown'

unknown_host2 = 'unknown2'
    ...
    tests = [
        dict(
            ...
            expected=errors.NotFound(
                reason='%s: netgroup not found' % netgroup1
            ),
        ),
        dict(
            ...
            command=(
                'netgroup_mod',
                [netgroup1],
                dict(description='Updated hostgroup 1'),
            ),
            expected=errors.NotFound(
                reason='%s: netgroup not found' % netgroup1
            ),
        ...
        dict(
            ...
            command=(
                'netgroup_add',
                [netgroup1],
                dict(description='Test', nisdomainname=invalidnisdomain1),
            ),
            expected=errors.ValidationError(
                name='nisdomain',
                error='may only include letters, numbers, _, -, and .',
        ...
        dict(
            ...
            command=(
                'netgroup_add',
                [netgroup1],
                dict(description='Test', nisdomainname=invalidnisdomain2),
            ),
            expected=errors.ValidationError(
                name='nisdomain',
                error='may only include letters, numbers, _, -, and .',
        ...
        dict(
            ...
            expected=dict(
                value=netgroup1,
                summary='Added netgroup "%s"' % netgroup1,
                result=dict(
                    dn=fuzzy_netgroupdn,
                    cn=[netgroup1],
        ...
        dict(
            ...
            command=(
                'netgroup_add',
                [netgroup2],
                dict(description='Test netgroup 2'),
            ),
            expected=dict(
                value=netgroup2,
                summary='Added netgroup "%s"' % netgroup2,
                result=dict(
                    dn=fuzzy_netgroupdn,
                    cn=[netgroup2],
                    objectclass=objectclasses.netgroup,
        ...
        dict(
            ...
            command=(
                'netgroup_add',
                [netgroup_single],
                dict(description='Test netgroup_single'),
            ),
            expected=dict(
                value=netgroup_single,
                summary='Added netgroup "%s"' % netgroup_single,
                result=dict(
                    dn=fuzzy_netgroupdn,
                    cn=[netgroup_single],
                    objectclass=objectclasses.netgroup,
                    description=['Test netgroup_single'],
                    nisdomainname=['%s' % api.env.domain],
                    ipauniqueid=[fuzzy_uuid],
                ),
            ),
        ...
        dict(
            ...
            expected=dict(
                value=[netgroup_single],
                summary='Deleted netgroup "%s"' % netgroup_single,
                result=dict(failed=[]),
            ),
        ),
        dict(
            ...
            expected=errors.DuplicateEntry(
                message='netgroup with name "%s" already exists' % netgroup1
            ),
        ),
        dict(
        ...
        dict(
            desc='Create %r' % user1,
            command=('user_add', [user1], dict(givenname='Test', sn='User1')),
            expected=dict(
                value=user1,
        ...
        dict(
            desc='Create %r' % user2,
            command=('user_add', [user2], dict(givenname='Test', sn='User2')),
            expected=dict(
                value=user2,
                summary='Added user "%s"' % user2,
                result=get_user_result(user2, 'Test', 'User2', 'add'),
            ),
        ),
        dict(
        ...
        dict(
            ...
            expected=errors.ValidationError(
                name='host',
                error="only letters, numbers, '_', '-' are allowed. "
        ...
        dict(
            ...
            command=(
                'netgroup_add_member',
                [netgroup1],
                dict(netgroup='notfound'),
            ),
            expected=dict(
                completed=0,
                failed=dict(
                    member=dict(
                        netgroup=[('notfound', 'no such entry')],
                    ),
                    memberuser=dict(
                        group=tuple(),
                        user=tuple(),
        ...
        dict(
            ...
            expected=dict(
                ...
                failed=dict(
                    ...
                    memberuser=dict(
                        group=tuple(),
                        user=[('%s' % user1, 'This entry is already a member')],
                    ),
                    memberhost=dict(
                        hostgroup=tuple(),
                        host=tuple(),
        ...
        dict(
            ...
            expected=dict(
                ...
                failed=dict(
                    ...
                    memberuser=dict(
                        group=[
                            ('%s' % group1, 'This entry is already a member')
                        ],
                        user=tuple(),
                    ),
                    memberhost=dict(
        ...
        dict(
            ...
            expected=dict(
                ...
                failed=dict(
                    ...
                    memberhost=dict(
                        hostgroup=tuple(),
                        host=[('%s' % host1, 'This entry is already a member')],
                    ),
                ),
                result={
                    'dn': fuzzy_netgroupdn,
        ...
        dict(
            ...
            expected=dict(
                value='netgroup1',
                summary='Modified netgroup "netgroup1"',
                result={
                    'memberhost_host': (host1,),
                    'memberhost_hostgroup': (hostgroup1,),
                    'memberuser_user': (user1,),
        ...
        dict(
            ...
            command=('netgroup_find', [], dict(cn=netgroup1, no_members=False)),
            expected=dict(
                count=1,
                truncated=False,
                summary='1 netgroup matched',
        ...
        dict(
            desc=(
                'Search for all netgroups using empty member user with members'
            ),
        ...
        dict(
            ...
            command=('netgroup_remove_member', [netgroup1], dict(group=group1)),
            expected=dict(
                completed=1,
                failed=dict(
                    member=dict(
        ...
        dict(
            ...
            expected=dict(
                ...
                failed=dict(
                    ...
                    memberhost=dict(
                        hostgroup=[
                            ('%s' % hostgroup1, 'This entry is not a member')
                        ],
                        host=tuple(),
                    ),
                ),
        ...
        dict(
            ...
            expected=dict(
                ...
                failed=dict(
                    member=dict(
                        netgroup=[
                            ('%s' % netgroup2, 'This entry is not a member')
                        ],
                    ),
                    memberuser=dict(
                        group=tuple(),
        ...
        dict(
            ...
            expected=dict(
                value=[netgroup1],
                summary='Deleted netgroup "%s"' % netgroup1,
                result=dict(failed=[]),
            ),
        ),
    ]
