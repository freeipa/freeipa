...
class TestTopologySuffix(XMLRPC_test):
    ...
    def test_verify_domain_suffix(self):
        ...
        r = result['result']
        assert r['in_order']
        assert r['connect_errors'] == []
        assert r['max_agmts_errors'] == []
...
class TestTopologySegment(XMLRPC_test):
    ...
    def test_segment_find(self):
        """topologysegment_find on the domain suffix must succeed."""
        result = self.tracker.run_command(
            'topologysegment_find', DOMAIN_SUFFIX_NAME)
        # On a single-master install there are no segments; the command
        # must still return successfully with an empty (or non-empty) list.
