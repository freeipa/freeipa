...
from ipatests.util import (
    assert_equal, assert_not_equal, unlock_principal_password
)
...
sysaccount1 = 'my-app'
testuser = 'test_user'
testuser_oldpw = 'Secret123!4'
testuser_newpw = 'Secret123!5'
    ...
    def test_sysaccount_use(self, test_user):
        ...
        batch = [
            ...
            dict(method='role_add_member', params=([role],
                 dict(sysaccounts=self.account))),
            dict(method='sysaccount_mod', params=([self.account],
                 dict(random=True))),
            dict(method='sysaccount_allow_reset', params=([self.account],
                 dict(update_without_reset=True))),
        ]
        ...
        assert_equal(
            ...
            messages.ServerPasssyncMgrUpdateRequired.errno
        )
        ...
        unlock_principal_password(test_user.name,
                                  testuser_oldpw, testuser_newpw)
        ...
        args = [paths.LDAPPASSWD, '-D', str(self.dn),
                '-w', self.passwd, '-a', testuser_newpw, '-s', self.passwd,
        ...
        conn = ldap2(api)
        conn.connect(bind_dn=test_user.dn, bind_pw=self.passwd)
        conn.disconnect()
        ...
        batch = [
            ...
            dict(method='sysaccount_allow_reset', params=([self.account],
                 dict(update_without_reset=False))),
