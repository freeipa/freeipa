...
class test_sysaccount(UI_driver):
    ...
    def restart_dirsrv(self):
        ...
        # Try config keys first, then check if default paths exist
        private_key_path = (
            self.config.get('root_ssh_key_filename')
            or self.config.get('ssh_private_key_path')
        )

        # If not in config, check if default paths exist
        if not private_key_path:
            prci_path = "/root/.ssh/freeipa_pr_ci_insecure"
            idmci_path = os.path.expanduser("~/.ssh/id_rsa")
            if os.path.exists(prci_path):
                private_key_path = prci_path
            elif os.path.exists(idmci_path):
        ...
        if realm:
            dashed_domain = realm.replace(".", "-")
            restart = f"systemctl restart dirsrv@{dashed_domain}"
            tasks.run_ssh_cmd(
                to_host=hostname, username='root',
                auth_method="key",
