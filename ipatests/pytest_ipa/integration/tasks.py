...
def del_a_record(master, zone, name, a_rec=None, raiseonerr=True):
    """Delete A record."""
    command = ['ipa', 'dnsrecord-del', zone, name]
    if a_rec:
        if isinstance(a_rec, list):
            for rec in a_rec:
                command.extend(['--a-rec', rec])
        else:
            command.extend(['--a-rec', a_rec])
    return master.run_command(command, raiseonerr=raiseonerr)


def find_a_record(master, zone, name, raiseonerr=True, **kwargs):
    """Find A record with optional filters.

    :param master: The IPA master host to run command on
    :param zone: DNS zone name (e.g., 'example.com.')
    :param name: Record name (e.g., 'www')
    :param raiseonerr: If True, raise exception on command failure
    :param kwargs: Additional filters (e.g., a='192.168.1.2' becomes
                   --a-rec 192.168.1.2)
    :return: Command result object

    Example:
        find_a_record(master, zone, 'www', a='192.168.1.2')
        # runs: ipa dnsrecord-find zone www --a-rec 192.168.1.2
    """
    command = ['ipa', 'dnsrecord-find', zone, name]
    # Add kwargs as filters (e.g., a='1.2.3.4' becomes --a-rec 1.2.3.4)
    for key, value in kwargs.items():
        command.extend([f'--{key}-rec', str(value)])
    return master.run_command(command, raiseonerr=raiseonerr)
...
def add_dns_record(master, zone, name, record_type, record_value,
    ...
    :param record_type: Record type like 'a', 'aaaa', 'afsdb', 'cname',
                        ...
                        'cert', 'loc', 'kx', etc.
    :param record_value: Single value or list of values for the record
    :param raiseonerr: If True, raise exception on command failure
    :return: Command result object
    """
    # Build the dnsrecord-add command
    command = ['ipa', 'dnsrecord-add', zone, name]
    # Add record type option (e.g., --a-rec, --aaaa-rec)
    ...
    if isinstance(record_value, list):
        ...
        command.extend([opt, record_value])
    return master.run_command(command, raiseonerr=raiseonerr)
...
def del_dns_record(master, zone, name, record_type=None, record_value=None,
    ...
    :param record_value: Single value or list of values (optional)
