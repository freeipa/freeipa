...
class test_otptoken_import:
    def test_figure3(self):
        ...
        assert [(t.id, t.options) for t in doc.getKeyPackages()] == \
            [('12345678', {
                ...
                'ipatokenvendor': 'Manufacturer',
                'ipatokenserial': '987654321',
    ...
    def test_figure7(self):
        ...
        assert [(t.id, t.options) for t in doc.getKeyPackages()] == \
            [('123456', {
                'ipatokenotpkey': 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ',
                'ipatokenvendor': 'TokenVendorAcme',
                'ipatokenserial': '987654321',
                'ipatokenotpdigits': 8,
                'type': 'hotp'})]
    ...
    def test_full(self):
        ...
        assert [(t.id, t.options) for t in doc.getKeyPackages()] == \
            [('KID1', {
                'ipatokenotpkey': 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ',
                'ipatokennotafter': '20060531000000Z',
                'ipatokennotbefore': '20060501000000Z',
                'ipatokenserial': 'SerialNo-IssueNo',
                'ipatokentotpclockoffset': 60000,
                'ipatokenotpalgorithm': 'sha1',
                'ipatokenvendor': 'iana.dummy',
    ...
    def test_valid_tokens(self):
        assert convertHashName('sha1') == 'sha1'
        assert convertHashName('hmac-sha1') == 'sha1'
        assert convertHashName('sha224') == 'sha224'
        assert convertHashName('hmac-sha224') == 'sha224'
        assert convertHashName('sha256') == 'sha256'
        assert convertHashName('hmac-sha256') == 'sha256'
        assert convertHashName('sha384') == 'sha384'
        assert convertHashName('hmac-sha384') == 'sha384'
        assert convertHashName('sha512') == 'sha512'
        assert convertHashName('hmac-sha512') == 'sha512'

    def test_invalid_tokens(self):
        """The conversion defaults to sha1 on unknown hashing"""
        assert convertHashName('something-sha256') == 'sha1'
        assert convertHashName('') == 'sha1'
        assert convertHashName(None) == 'sha1'
