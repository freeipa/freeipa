...
def do_nsupdate(update_txt, dns_over_tls=False):
    ...
    try:
        if dns_over_tls:
            ipautil.run([paths.NSUPDATE, '-p', '853', '-S', '-g', UPDATE_FILE])
        else:
        ...
        try:
            if dns_over_tls:
                ipautil.run([paths.NSUPDATE, '-p', '853', '-S', UPDATE_FILE])
            else:
...
def update_dns(server, hostname, options):
    ...
    if not do_nsupdate(update_txt, options.dns_over_tls):
...
def update_ssh_keys(hostname, ssh_dir, create_sshfp, dns_over_tls=False):
    ...
    if create_sshfp:
        ...
        if not do_nsupdate(update_txt, dns_over_tls):
